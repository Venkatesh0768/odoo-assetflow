...
const migrations = [
  /* ─── 001 – users (updated with new roles + employee fields) ─────────── */
  `
  ...
  /* Add FK from users.department_id → departments after departments table exists */
  `
  DO $$
  BEGIN
    IF NOT EXISTS (
  ...
  /* ─── 016 – updated_at trigger function ────────────────────────────── */
  `
  ...
  ...['users','departments','asset_categories','assets','allocations',
      'transfer_requests','bookings','maintenance_requests','audit_cycles'].map(t => `
  DO $$
  BEGIN
    IF NOT EXISTS (
